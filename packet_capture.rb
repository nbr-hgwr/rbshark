...
if ARGV.size != 1
  STDERR.puts "capture <device-name>"
  return 1
end
...
socket = Socket.open(Socket::AF_PACKET, Socket::SOCK_RAW, ETH_P_ALL)
ifreq = ARGV
ifreq = ifreq.dup.pack('a' + IFREQ_SIZE.to_s)
...
while (true)
  ...
  frame = mesg[0]
  ether_header = EthernetAnalyzer.new(frame)
  print_ethernet(ether_header)
  case ether_header.check_protocol_type
  when "ARP"
    arp_header = ARPAnalyzer.new(frame, ether_header.get_byte)
    print_arp(arp_header)
  when "IP"
    ip_header = IPAnalyzer.new(frame, ether_header.get_byte)
    print_ip(ip_header)
    case ip_header.check_protocol_type
    when "ICMP"
      icmp = ICMPAnalyzer.new(frame, ip_header.get_byte)
      print_icmp(icmp)
    when "TCP"
      tcp = TCPAnalyzer.new(frame, ip_header.get_byte)
      print_tcp(tcp)
    when "UDP"
      udp = UDPAnalyzer.new(frame, ip_header.get_byte)
      print_udp(udp)
    end
  #when "IPv6"
